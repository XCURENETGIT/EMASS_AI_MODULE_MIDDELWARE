...
# ── 실행 모드 (dev / prod) ───────────────────────────────────
# 이 폴더(dev)는 개발 실행과 '프로덕션 이미지 빌드'까지만 담당한다.
#   - dev 모드 : 소스를 마운트해 개발용으로 실행 (compose 사용)
#   - prod 모드: .so 이미지를 '빌드/반출'만 한다 (Dockerfile.prod 직접 사용, compose 불필요)
#              예) ./manage.sh prod build   ./manage.sh prod save
# 프로덕션 '실행/운영'은 현장 폴더(deploy/)의 prod-manage.sh 로 한다.
COMPOSE_FILE="$PROJECT_DIR/docker-compose.yaml"   # dev 전용 compose
PROD_DOCKERFILE="$PROJECT_DIR/Dockerfile.prod"    # prod 이미지 빌드 레시피
MODE="dev"
IMAGE_TAG="emass-ai-middelware:0.0.0"     # dev 이미지 태그
if [ "${1:-}" = "prod" ]; then
  MODE="prod"
  IMAGE_TAG="emass-ai-middelware:prod"    # prod(.so) 이미지 태그
  shift
fi
...
# 이미지 빌드
#   dev  : compose 로 개발 이미지 빌드
#   prod : Dockerfile.prod 로 .so 컴파일 이미지 빌드 (compose 불필요)
cmd_build() {
  if [ "$MODE" = "prod" ]; then
    echo "🔨 프로덕션(.so) 이미지 빌드... ($IMAGE_TAG)"
    docker build -f "$PROD_DOCKERFILE" -t "$IMAGE_TAG" "$PROJECT_DIR"
    echo "✅ 빌드 완료. 반출: ./manage.sh prod save"
  else
    echo "🔨 이미지 빌드..."
    $DC build
  fi
...
# 상태 확인 (컨테이너 = 워커)
cmd_status() {
  echo "🧭 모드: $MODE  (compose: $(basename "$COMPOSE_FILE"))"
  if is_container_up; then
    echo "🟢 워커(컨테이너): 실행 중"
  else
    echo "🔴 워커(컨테이너): 중지됨"
...
# ── 프로덕션 이미지 반출 (배포지엔 이미지만 들고 나감) ──────────
# 사내에서 빌드한 .so 이미지를 파일로 내보낸다. (현장 deploy/ 에서 load 해서 사용)
#   ./manage.sh prod save                 → emass-ai-middelware-prod.tar.gz
#   ./manage.sh prod save /path/out.tar.gz
cmd_save() {
  local out="${1:-emass-ai-middelware-prod.tar.gz}"
  case "$out" in /*) ;; *) out="$PROJECT_DIR/$out" ;; esac
  if [ -z "$(docker images -q "$IMAGE_TAG" 2>/dev/null)" ]; then
    echo "❌ 이미지가 없습니다: $IMAGE_TAG" >&2
    echo "   먼저 빌드하세요: ./manage.sh prod build" >&2
    exit 1
  fi
  echo "📦 이미지 반출: $IMAGE_TAG → $out"
  docker save "$IMAGE_TAG" | gzip > "$out"
  echo "✅ 완료 ($(du -h "$out" | cut -f1))"
  echo "   이 파일을 배포용 폴더(deploy/)로 옮긴 뒤, 현장에서:"
  echo "     ./prod-manage.sh load $(basename "$out")  →  ./prod-manage.sh up"
}
...
usage() {
  cat <<EOF
emass-ai-middleware 관리 스크립트 (dev — 개발 실행 + 프로덕션 이미지 빌드)

사용법:
  ./manage.sh <명령>          # 개발 모드 (소스 마운트로 실행)
  ./manage.sh prod <명령>     # 프로덕션 이미지 빌드/반출 (실행/운영은 deploy/ 에서)
...
── 개발 모드 ──
  ...
  logs       워커 출력 실시간 보기
  log        날짜별 처리 로그 파일 보기 (log / log 3 / log 2026-06-17 / log list / log 3 -f)
  shell      컨테이너 안 셸 진입
  help       이 도움말

── 프로덕션 (prod 접두) ──  ※ 빌드·반출만. 실행/운영은 deploy/prod-manage.sh
  prod build   .so 컴파일 이미지 빌드 (Dockerfile.prod)
  prod save    빌드한 이미지를 파일로 반출 → emass-ai-middelware-prod.tar.gz
               (이 파일을 deploy/ 로 옮겨 현장에서 load)
EOF
}

# ── 디스패치 ─────────────────────────────────────────────────
if [ "$MODE" = "prod" ]; then
  # 프로덕션: 이미지 빌드/반출만 담당한다. (실행/운영은 deploy/prod-manage.sh)
  case "${1:-help}" in
    build)   cmd_build ;;
    save)    shift; cmd_save "$@" ;;
    help|-h|--help) usage ;;
    up|down|run|start|stop|restart|status|logs|log|shell|load)
      echo "❌ 'prod $1' 은 여기서 지원하지 않습니다. (dev 는 prod 이미지 빌드/반출만)" >&2
      echo "   프로덕션 실행/운영은 현장 폴더에서:  cd ../deploy && ./prod-manage.sh $1" >&2
      exit 1 ;;
    *) echo "알 수 없는 명령: prod $1"; echo; usage; exit 1 ;;
  esac
else
  # 개발 모드
  case "${1:-help}" in
